...
An ActionScript library (or swc) which contains the ActionScript code which calls into the native code.
The native code: On iOS one needs to create a CocoaTouch static library containing the native code which is then combined with the SWC (in step 1) and packaged into an ANE (AIR Native Extension) using the ADT tool.
The AIR app which uses the ANE generated in step 2.
...
In the attached zip (AIR Native Extension.zip) there is a bash script "install_templates.sh". From the command line:
...
Now launch XCode, click File->New->New Project (Command+Shift+N). You would see a new entry for "AIR Native Extension" under iOS:
new_project.png
Click Next and you would see the following options for configuring the XCode project.
"options.png"
...
Click Next and you would be asked to select a folder where you want to create your XCode project and keep your native code.
Click Next and XCode would create a project for you.
created_project.png
